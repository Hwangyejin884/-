...
void Store::Sales() { //case4의 판매 기록 관리
	...
	if (yes_no == 'Y') { //매장 회원일 때
		...
		if (yes_no == 'Y') {
			...
			cout << save_price * 0.05 << "포인트가 적립되어 " << customer[customer_index].name
				<< "님의 포인트가 " << customer[customer_index].point << "점이 되었습니다."<< endl;
			customer_index++;
			cout << "■ 결제하실 금액은 " << save_price << "원입니다. ■" << endl;
...
void Store::Check_SaleProduct() { //case5의 누적 판매량 확인 기능
	...
	case 2: //개별 명단 확인
		...
		cout << "※※ 일치하는 제품명이 없습니다. 초기화면으로 돌아갑니다. ※※" << endl << endl;
	default:
		cout << "┗ number는 1, 2 만 입력하실 수 있습니다. " << endl;
	}
}
